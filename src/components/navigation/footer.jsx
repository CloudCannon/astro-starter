...
export default function Footer({}) {
  return (
    <footer
      className="max-w-[1280px] mx-auto header px-6 md:px-12 footer mx-auto grid gap-3 lg:grid-cols-3 lg:items-center justify-center text-center 2xl:pt-19 pt-8 pb-5 sm:pb-7 lg:pt-20"
      id="footer">
      <a href={footer.logo_url} className="flex justify-center lg:col-start-2">
        <img className="logo" src={footer.logo} alt="logo" />
      </a>
      <p className="lg:col-start-1 lg:row-start-1 lg:text-left my-4">
        ...
        <ul className="flex gap-5 justify-center">
          {footer.social.map((link, i) => (
            <li key={i}>
              <a href={`${link.link}`}>
                <i
                  className={`${link.social_icon} text-2xl`}
                  aria-hidden="true">
